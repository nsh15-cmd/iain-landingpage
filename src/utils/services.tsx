export const services = [
  {
    title: "Virtual Interviews",
    description: "Conduct seamless video interviews anytime, anywhere.",
    icon: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
        viewBox="0 3 23 19"
        strokeWidth="1"
        stroke="currentColor"
        className="w-6 h-6"
      >
        <path
          xmlns="http://www.w3.org/2000/svg"
          d="M2.94414 8.28418C3.31747 6.62497 4.60448 5.31882 6.26653 4.91238C8.53218 4.35834 10.9252 4.36467 13.1909 4.91871C14.832 5.32005 16.1361 6.59343 16.549 8.22135L16.574 8.31991C17.1868 10.7356 17.1868 13.2644 16.574 15.6801L16.549 15.7787C16.1361 17.4066 14.832 18.68 13.1909 19.0813C10.9252 19.6353 8.53218 19.6417 6.26653 19.0876C4.60449 18.6812 3.31747 17.375 2.94414 15.7158L2.8825 15.4419C2.3725 13.1752 2.3725 10.8248 2.8825 8.55815L2.94414 8.28418Z"
          stroke-width="1.5"
        />
        <path
          xmlns="http://www.w3.org/2000/svg"
          d="M16.6633 15.3084L16.8603 15.3736C16.9115 15.3905 16.9614 15.4111 17.0097 15.4351L18.6936 16.2713C19.9829 16.9117 21.5 15.9804 21.5 14.5485L21.5 9.81013C21.5 8.29729 19.8246 7.37498 18.533 8.17679L17.0474 9.09908C16.9742 9.14452 16.9064 9.19808 16.8453 9.25871L16.7904 9.31325C17.1526 11.2977 17.1102 13.3366 16.6633 15.3084Z"
          stroke-width="1.5"
        />
      </svg>
    ),
  },
  {
    title: "Messaging & Chat",
    description: "Get real-time communication from our webb-app.",
    icon: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
        viewBox="0 0 16 20"
        strokeWidth="1"
        stroke="currentColor"
        className="w-6 h-6"
      >
        <path
          xmlns="http://www.w3.org/2000/svg"
          id="Path_148"
          data-name="Path 148"
          d="M-20.5,16a.5.5,0,0,1-.354-.146.5.5,0,0,1-.131-.475l.912-3.648A8,8,0,0,1-21,8a8.009,8.009,0,0,1,8-8A8.009,8.009,0,0,1-5,8a8.009,8.009,0,0,1-8,8,7.993,7.993,0,0,1-3.731-.927l-3.648.912A.5.5,0,0,1-20.5,16Zm3.836-1.959a.506.506,0,0,1,.245.064A7.007,7.007,0,0,0-13,15,7.008,7.008,0,0,0-6,8a7.008,7.008,0,0,0-7-7,7.008,7.008,0,0,0-7,7,7.01,7.01,0,0,0,.9,3.419.5.5,0,0,1,.049.366l-.757,3.028,3.028-.757A.5.5,0,0,1-16.664,14.041Z"
          transform="translate(21)"
        />
      </svg>
    ),
  },
  {
    title: "Easy Scheduling",
    description: "Set up and manage interviews with just a few clicks.",
    icon: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
        viewBox="0 0 24 24"
        strokeWidth="1.5"
        stroke="currentColor"
        className="w-6 h-6"
      >
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
          d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5"
        />
      </svg>
    ),
  },
];
